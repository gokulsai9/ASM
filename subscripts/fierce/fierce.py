...
def print_subdomain_result(url, ip, http_connection_headers=None, nearby=None):
    result = {
        "Found": {"name": url, "ip": ip},
        "Nearby": nearby if nearby else {}
    }

    if http_connection_headers:
        result["HTTP_Connection_Headers"] = http_connection_headers
    return result
...
def find_subdomain_list_file(filename):
    # First check the list directory relative to where we are.
    filename_path = os.path.join(os.path.dirname(__file__), "lists", filename)
    if os.path.exists(filename_path):
        return os.path.abspath(filename_path)

    try:
        # Use importlib.resources.files to locate files within the package
        files = importlib.resources.files('fierce.lists')
        full_package_path = files.joinpath(filename)
        if full_package_path.exists():
            return str(full_package_path)
    except ImportError:
        return filename
...
def fierce(**kwargs):
    ...
    if ns:
        domain_name_servers = [str(n) for n in ns]
    else:
        domain_name_servers = []

    results["NS"] = domain_name_servers if ns else "failure"
    ...
    if soa:
        ...
        if master:
            ...
            results["SOA"] = {"name": str(soa_mname), "ip": str(master_address)}
        else:
            results["SOA"] = "failure"
    ...
    results["Zone"] = "success" if zone else "failure"
    if zone:
        zone_dict = {}
        for k, v in zone.items():
            zone_dict[str(k)] = v.to_text(k)
        results.update(zone_dict)
        return

    random_subdomain = str(random.randint(10**10, 10**11 - 1))
    ...
    results["Wildcard"] = ', '.join(wildcard_ips) if wildcard_ips else "failure"
    ...
    for subdomain in subdomains:
        try:
            ...
            print("Error occurred for subdomain: " + str(subdomain) + ". Skipping.")
            continue
    ...
    if output_file and os.path.exists(output_file):
        ...
        results["Found"].extend(existing_data.get("Found", []))
        results["Nearby"].update(existing_data.get("Nearby", {}))
